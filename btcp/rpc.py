...
#
# This file is part of python-btcplib.
#
# It is subject to the license terms in the LICENSE file found in the top-level
# directory of this distribution.
#
# No part of python-btcplib, including this file, may be copied, modified,
...
import btcp
from btcp.core import COIN, x, lx, b2lx, CBlock, CBlockHeader, CTransaction, COutPoint, CTxOut
...
class BaseProxy(object):
    ...
    def __init__(self,
        ...
        if service_url is None:
            # Figure out the path to the btcprivate.conf file
            if btcp_conf_file is None:
                if platform.system() == 'Windows':
                    btcp_conf_file = os.path.join(os.environ['APPDATA'], 'btcp')
                else:
                    btcp_conf_file = os.path.expanduser('~/.btcprivate')
                btcp_conf_file = os.path.join(btcp_conf_file, 'btcprivate.conf')

            # Extract contents of btcprivate.conf to build service_url
            with open(btcp_conf_file, 'r') as fd:
                # btcp accepts empty rpcuser, not specified in btcp_conf_file
                conf = {'rpcuser': ""}
                ...
                if service_port is None:
                    service_port = btcp.params.RPC_PORT
                conf['rpcport'] = int(conf.get('rpcport', service_port))
                conf['rpchost'] = conf.get('rpcconnect', 'localhost')

                if 'rpcpassword' not in conf:
                    raise ValueError('The value of rpcpassword not specified in the configuration file: %s' % btcp_conf_file)
...
class RawProxy(BaseProxy):
    """Low-level proxy to a btcp JSON-RPC service
    ...
    def __getattr__(self, name):
        ...
        # Make debuggers show <function btcp.rpc.name> rather than <function
        # btcp.rpc.<lambda>>
        f.__name__ = name
        return f
...
class Proxy(BaseProxy):
    """Proxy to a btcp RPC service

    Unlike ``RawProxy``, data is passed as ``btcp.core`` objects or packed
    ...
    way. Assumes btcp version >= v1.0.0
    """
    ...
    def __init__(self,
        ...
        out of the file ``btcp_conf_file``. If ``btcp_conf_file`` is not
        specified, ``~/.btcprivate/btcprivate.conf`` or equivalent is used by
        default.  The default port is set according to the chain parameters in
        use: mainnet, testnet, or regtest.

        Usually no arguments to ``Proxy()`` are needed; the local btcpd will
        be used.
    ...
    def getaccountaddress(self, account=None):
        """Return the current btcp address for receiving payments to this
        account."""
    ...
    def getnewaddress(self, account=None):
        """Return a new btcp address for receiving payments.
    ...
    def submitblock(self, block, params=None):
        ...
        https://en.btcprivate.it/wiki/BIP_0022 for full specification.
        """
