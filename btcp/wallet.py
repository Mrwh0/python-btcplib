...
import btcp.base58
import btcp.core
import btcp.core.key
import btcp.core.script as script

class CBitcoinAddress(btcp.base58.CBase58Data):
    """A btcp transparent address"""
    @classmethod
    def from_bytes(cls, data, nVersion):
        self = super(CBitcoinAddress, cls).from_bytes(data, nVersion)
        if nVersion == btcp.params.BASE58_PREFIXES['SCRIPT_ADDR']:
            self.__class__ = P2SHBitcoinAddress

        elif nVersion == btcp.params.BASE58_PREFIXES['PUBKEY_ADDR']:
            self.__class__ = P2PKHBitcoinAddress

        else:
           raise CBitcoinAddressError('Version {0} not a recognized btcp Address'.format(nVersion))
...
class P2SHBitcoinAddress(CBitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = btcp.params.BASE58_PREFIXES['SCRIPT_ADDR']

        elif nVersion != btcp.params.BASE58_PREFIXES['SCRIPT_ADDR']:
    ...
    def from_scriptPubKey(cls, scriptPubKey):
        ...
        if scriptPubKey.is_p2sh():
            # are there 23 bytes here for btcp?
            return cls.from_bytes(scriptPubKey[2:22], btcp.params.BASE58_PREFIXES['SCRIPT_ADDR'])
    ...
    def to_scriptPubKey(self):
        """Convert an address to a scriptPubKey"""
        assert self.nVersion == btcp.params.BASE58_PREFIXES['SCRIPT_ADDR']
        return script.CScript([script.OP_HASH160, self, script.OP_EQUAL])

class P2PKHBitcoinAddress(CBitcoinAddress):
    # convert a bitcoin P2PKH address to the btcp one with same public key hash
    @classmethod
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = btcp.params.BASE58_PREFIXES['PUBKEY_ADDR']
    ...
    def from_pubkey(cls, pubkey, accept_invalid=False):
        """Create a P2PKH btcp address from a pubkey
        ...
        if not accept_invalid:
            if not isinstance(pubkey, btcp.core.key.CPubKey):
                pubkey = btcp.core.key.CPubKey(pubkey)
            if not pubkey.is_fullyvalid:
                raise CBitcoinAddressError('invalid pubkey')

        pubkey_hash = btcp.core.Hash160(pubkey)
    ...
    def from_scriptPubKey(cls, scriptPubKey, accept_non_canonical_pushdata=True, accept_bare_checksig=True):
        ...
        if accept_non_canonical_pushdata:
            ...
            except btcp.core.script.CScriptInvalidError:
        ...
        if (len(scriptPubKey) == 25
            ...
            return cls.from_bytes(scriptPubKey[3:23], btcp.params.BASE58_PREFIXES['PUBKEY_ADDR'])
    ...
    def to_scriptPubKey(self):
        """Convert an address to a scriptPubKey"""
        assert self.nVersion == btcp.params.BASE58_PREFIXES['PUBKEY_ADDR']
...
class CKey(object):
    ...
    def __init__(self, secret, compressed=True):
        self._cec_key = btcp.core.key.CECKey()
        self._cec_key.set_secretbytes(secret)
        self._cec_key.set_compressed(compressed)

        self.pub = btcp.core.key.CPubKey(self._cec_key.get_pubkey(), self._cec_key)
...
class CBitcoinSecretError(btcp.base58.Base58Error):
    pass

class CBitcoinSecret(btcp.base58.CBase58Data, CKey):
    ...
    def from_secret_bytes(cls, secret, compressed=True):
        ...
        self = cls.from_bytes(secret + (b'\x01' if compressed else b''),
                              btcp.params.BASE58_PREFIXES['SECRET_KEY'])
        self.__init__(None)
        return self

    def __init__(self, s):
        if self.nVersion != btcp.params.BASE58_PREFIXES['SECRET_KEY']:
            raise CBitcoinSecretError('Not a base58-encoded secret key: got nVersion={0}; expected nVersion={1}'.format(self.nVersion, btcp.params.BASE58_PREFIXES['SECRET_KEY']))
