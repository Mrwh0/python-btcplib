...
from btcp.core.key import CPubKey
from btcp.core.serialize import ImmutableSerializable
from btcp.wallet import P2PKHBitcoinAddress
import btcp
import base64
import sys
...
class BitcoinMessage(ImmutableSerializable):
    ...
    def stream_deserialize(cls, f):
        magic = btcp.core.serialize.BytesSerializer.stream_deserialize(f)
        message = btcp.core.serialize.BytesSerializer.stream_deserialize(f)
        return cls(message, magic)

    def stream_serialize(self, f):
        btcp.core.serialize.BytesSerializer.stream_serialize(self.magic, f)
        btcp.core.serialize.BytesSerializer.stream_serialize(self.message, f)
