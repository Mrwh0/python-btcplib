...
from btcp.wallet import CBitcoinSecret, P2PKHBitcoinAddress
from btcp.signmessage import BitcoinMessage, VerifyMessage, SignMessage
...
class Test_SignVerifyMessage(unittest.TestCase):
    def test_verify_message_simple(self):
        ...
        address = btcp.wallet.P2PKHBitcoinAddress.from_bitcoin_address(message)
    ...
    def test_verify_message_vectors(self):
        for vector in load_test_vectors('signmessage.json'):
            ...
            address = btcp.wallet.P2PKHBitcoinAddress.from_bitcoin_address(vector['address'])
    ...
    def test_sign_message_vectors(self):
        for vector in load_test_vectors('signmessage.json'):
            ...
            #message = BitcoinMessage(btcp.wallet.P2PKHBitcoinAddress.from_bitcoin_address("1K5Z1nxN4mjUgCLpSXMRkeZxuAMpbn2CQB").__str__())
            message = BitcoinMessage(vector['address'])

            address = btcp.wallet.P2PKHBitcoinAddress.from_bitcoin_address(vector['address'])
            # print("address:", address)
            signature = SignMessage(key, message)
            # address2 = btcp.wallet.P2PKHBitcoinAddress.from_privkey(key)
            # print()
