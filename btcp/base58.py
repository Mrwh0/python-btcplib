...
class CBase58Data(bytes):
    ...
    def __new__(cls, s):
        k = decode(s)
        """ In btcp private keys have one byte version numbers (same as bitcoin) but other base58 encoded objects have two bytes"""
        ...
        if (bytes(k[0:1]) == b'\xef' or  bytes(k[0:1])== b'\x80'):
            ...
            check1 = btcp.core.Hash(verbyte + data)[:4]
            if check0 != check1:
    ...
    def __str__(self):
        ...
        if type(self.nVersion) == int:
            ...
            # Appending two bytes btcp uses for nVersion
